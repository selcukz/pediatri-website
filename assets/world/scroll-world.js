/* ============================================================================
   ...
   (aslidoktor.com embedded build: segment math is offset by the track's
   document position and the fixed layers only show while the scroll band is
   in view, so the world can live mid-page as a homepage section.)
   ----------------------------------------------------------------------------
   Framework-agnostic. Vanilla JS, zero dependencies. It builds its own DOM and
   injects its own (namespaced) CSS into a container you give it.
...
function mountScrollWorld(container, config) {
  ...
  let trackTop = 0;         // document-space top of the scroll band (embedded offset)
  let isOn = false;         // fixed layers shown only while the band is in view

  function layout() {
    vh = window.innerHeight;
    laidOutW = window.innerWidth;
    stageX = window.innerWidth > 860 ? 4 : 0;
    trackTop = track.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
    let off = 0;
    SEGMENTS.forEach(s => { s.start = trackTop + off * vh; off += s.w; s.end = trackTop + off * vh; });
    totalW = off;
  ...
  function read() {
    ...
    // Embedded gate: fixed layers exist only while the band is on screen.
    const on = y > trackTop - 0.45 * vh && y < trackTop + totalW * vh + 0.05 * vh;
    if (on !== isOn) {
      isOn = on;
      container.classList.toggle('sw-on', on);
      document.documentElement.classList.toggle('sw-world-active', on);
    }
    ...
    scrollbarFill.style.transform = `scaleX(${clamp((y - trackTop) / (totalW * vh))})`;
    hint.style.opacity = clamp(1 - (y - trackTop) / (0.5 * vh));
    if (particles) particles.style.transform = `translate3d(0, ${-(y - trackTop) * 0.05}px, 0)`;
    ticking = false;
  }
  ...
  // iOS: prime muted videos on first gesture so seeks paint instead of staying blank.
  let userReady = false;
  ...
  // Ignore URL-bar-only resizes on touch (height-only change) — width change relayouts.
  function onResize() {
  ...
  // Embedded: content above the band (images, fonts) can change the track's
  // document offset after initial layout — relayout only when it actually moves,
  // so URL-bar height changes never yank the scroll position.
  if (window.ResizeObserver) {
    new ResizeObserver(() => {
      const t = track.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
      if (Math.abs(t - trackTop) > 2) layout();
  ...
  function ctaBtns(cta) {
    const ext = href => /^https?:/i.test(href || '') ? ' target="_blank" rel="noopener"' : '';
    let h = '';
    if (cta.primary) h += `<a class="sw-btn sw-btn--primary" href="${esc(cta.primary.href || '#')}"${ext(cta.primary.href)}>${esc(cta.primary.label)}</a>`;
    if (cta.secondary) h += `<a class="sw-btn sw-btn--ghost" href="${esc(cta.secondary.href || '#')}"${ext(cta.secondary.href)}>${esc(cta.secondary.label)}</a>`;
    return h;
  }
}
...
function injectCSS() {
  ...
  /* Embedded gate: every fixed layer stays hidden until the scroll band is on
     screen (.sw-on set by read()), so the world never covers the host page. */
  .sw-root .sw-sky,.sw-root .sw-scrollbar,.sw-root .sw-topbar,.sw-root .sw-stage,
  .sw-root .sw-copylayer,.sw-root .sw-route,.sw-root .sw-hint{
    opacity:0;visibility:hidden;transition:opacity .4s ease,visibility 0s linear .4s;}
  .sw-root.sw-on .sw-sky,.sw-root.sw-on .sw-scrollbar,.sw-root.sw-on .sw-topbar,
  .sw-root.sw-on .sw-stage,.sw-root.sw-on .sw-copylayer,.sw-root.sw-on .sw-route,
  .sw-root.sw-on .sw-hint{opacity:1;visibility:visible;transition:opacity .4s ease;}
